...
phyobject.hellrar <- transform_sample_counts(rarPhySeq, function(x) x^0.5)  ## I used rarPhyseq because it looked the best for ITS data
...
##Hellinger transform data with Square root for any phyloseq dataset
rarfun.hell <- transform_sample_counts(rarfun, function(x) x^0.5)  ## I used rarfun because it looked the best for ITS data
...
plot(fq2)
dev.copy(png, "figures/Heat_Order_Site", units="in", width=8, height=6, res=300)
dev.off()
...
PS_normal <- readRDS("data/PS_normal.gz") ## normalized phyloseq data
physeq1 <- readRDS("data/physeq1.gz")     ## phyloseq data
rarPhySeq <- readRDS("data/rarPhySeq.gz") ## rarified phyloseq data 
...
##Hellinger transform data with Square root for any phyloseq dataset
phyobject.hell <- transform_sample_counts(rarPhySeq, function(x) x^0.5)  ## I used rarPhyseq because it looked the best for ITS data

phyobject.hell1 <- transform_sample_counts(PS_normal, function(x) x^0.5)  ## I used rarPhyseq because it looked the best for ITS data
###############################################################################################
###Subset Data###
###############################################################################################
...
tiff("figures/16s_phylum_heatmap_PSnorm.tiff", units="in", width=12, height=12, res=300) ## Change "ITS_order_heatmap.tiff" to whatever title for the file you choose
plot(q2)
dev.off()
```
...
PS_normal <- readRDS("data/fungal_PS_normal.gz") ## normalized phyloseq data
physeq1 <- readRDS("data/fungal_physeq1.gz")     ## phyloseq data
rarPhySeq <- readRDS("data/fungal_rarPhySeq.gz") ## rarified phyloseq data 
...
##Save Heatmap as .tiff file 
tiff("figures/ITS_order_heatmap.tiff", units="in", width=12, height=12, res=300) ## Change "ITS_order_heatmap.tiff" to whatever title for the file you choose
plot(q2)
dev.off()
```

#### Volcano plot  
not yet resolved. Need code that generates the abundance table esv files. Once start code is in, the csvs should be put in the *output* folder and called from there.  

```{r, volcano plot, results="hide"}
...
f_allCounts <- read.csv("output/volcano/abundance_table_esv_volcanoPlot.csv", row.names=1, quote="")

b_allCounts <- read.csv("output/volcano/abundance_ESV_bacteria.csv", row.names=1, quote="")

#import metadata for fungi and bacteria
metadata<- read.csv("metadata/FIXED BOT662_Metadata.csv")

f_conditons<- metadata[(metadata$Locus=="fungalITS"),]
b_conditions <- metadata[(metadata$Locus=="bacterial16S"),]
...
taxonomy<- read.delim("metadata/consensus_taxonomy.txt", sep="")
fungi_taxonomy <- taxonomy[(taxonomy$Kingdom=="Fungi"), ]

bacteria_taxonomy <- read.table(file = 'metadata/taxonomic_assignments_esv.tsv', sep = '\t', header = TRUE)
bacteria_taxonomy <- bacteria_taxonomy[(bacteria_taxonomy$Kingdom=="Bacteria"),]
...
f_significant_classes <- c("Agaricomycetes","Dothideomycetes","Mortierellomycetes","Sordariomycetes","Tremellomycetes")
...
b_significant_classes <- c("Acidimicrobiia", "Anaerolineae","Blastocatellia","Chloroflexia", "Gemmatimondadetes","Latescibacteria_cl","NC10", "Oxyphotobacteria","Phycisphaerae","Planctomycetacia")
...
#Export plots
tiff(file="figures/VP.bacteria.classFacet.tiff",width=2100,height=1890,res=300)
...
tiff(file="figures/VP.fungi.classFacet.tiff",width=2100,height=1890,res=300)
...
#Export data
write.table(b_res.df.tax.significant, file="output/bacteria_parametricDESeq_significant.tsv", sep="\t",eol="\n")
write.table(f_res.df.tax.significant, file="output/fungi_parametricDESeq_significant.tsv", sep="\t",eol="\n")
```
