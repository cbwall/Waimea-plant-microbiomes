...
*1. The number of habitats vs. ASV range*  
*2. Mean abundance vs ASV range*  
*3. Mean habitat occurrence vs. ASV range*  
...
par(mfrow=c(3,2), mar=c(4,5,0,0.2))
...
#Set margin size for boxplot
# fungrange
boxplot(combined_rangespanF~Genus, horizontal = TRUE,  xaxt="n", yaxt="n", pch=21, cex=0.6, medlwd=1.5,
...
plot(jitter(rangespan,20),jitter(log(meanabund),3),col="gray48", cex=0.2, pch=16,
     ...
     cex.lab=0.6, xlab="ASV range (m)", ylab="Mean Abundance/Sample \n(Where Present)") + abline(lsfit(rangespan,log(meanabund)), col = "black", lwd = 1)
...
dev.print(png, "figures/ASV.range.png", units="in", width=6, height=4, res=300)
dev.off()
```
...
#Set margin size for boxplot
boxplot(rangespan, horizontal=TRUE, vertical=TRUE, col="#481567FF", main="Bacteria", frame=F, cex.main=0.8, xlab="ASV range (m)", cex.axis=0.8) 
#Second Plot: Fungal rangespan
#Set margin size for boxplot
boxplot(rangespanF, horizontal=TRUE  , col="#95D840FF", main="Fungi", frame=F, cex.main=0.8, xlab="ASV range (m)", cex.axis=0.8)
