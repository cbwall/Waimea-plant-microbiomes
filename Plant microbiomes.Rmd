...
author: "C Wall, J Bernard, and UH Mānoa BOT662, submitted ISME J"
date: "2/10/2020"
output:
  html_document:
    code_folding: hide
...
![**Figure 1. Site map with sampling locations**](figures/Fig1. Waimea map_color.jpg){ width=60%}

</center>  
  
  
#### Rarefaction: Bacteria and fungi  
Verify the sequence depth (~26,000 reads for bacteria and 20,000 for fungi) samples our communities well.  
  
```{r, rarefy richness, fig.dim=c(4,4), fig.align="center", fig.show="hold", fig.cap="Figure. Bacterial and Fungal rarefaction curves."}
#read in Bacteria data (not rarefied)
bactphyseq1=readRDS("data/physeq1.gz")
bactphyseq1 <- subset_taxa(bactphyseq1, Kingdom != "unknown")	#Remove unclassified kingdoms
bactphyseq1 <- subset_taxa(bactphyseq1, Kingdom != "Eukaryota")	#Remove eukaryotes
brich<-estimate_richness( bactphyseq1, measures="Observed")

# bacteria rarefaction curve
bac.rar<-rarecurve(otu_table(bactphyseq1), step=100, cex=0.5, label=FALSE, col="blue", main="Bacteria",
                   xlim=c(0, 300000))
abline(v = 26000, col="gray50", lwd=2, lty=2)

#read in Fungal data (not rarefied)
fungalphyseq1=readRDS("data/fungal_physeq1.gz")
fungalphyseq1 <- subset_taxa(fungalphyseq1, Kingdom != "unknown")	#Remove unclassified kingdoms
frich<-estimate_richness(fungalphyseq1, measures="Observed")

fun.rar<-rarecurve(otu_table(t(fungalphyseq1)), step=100, cex=0.5, label=FALSE, col="coral", main="Fungi",
                   xlim=c(0, 300000))
abline(v = 20000, col="gray50", lwd=2, lty=2)

# rarefy without replacement
fun.rarefied = rarefy_even_depth(fungalphyseq1, rngseed=1, sample.size=0.9*min(sample_sums(fungalphyseq1)), replace=F)
...
```{r, fungibacteria_mantel, results="hide", fig.dim=c(4,4), fig.align="center", fig.cap="Figure. Bacterial and Fungal community mantel tests."}
# Code by Anthony
...
**For Bacteria PHYLA**
```{r, taxonomy bacteria, results="hide", message=FALSE, fig.dim=c(6,8)}
...
**For Fungal CLASSES**
```{r, taxonomy fungi, results="hide", message=FALSE, fig.dim=c(6,8)}
######### Fungi 
...
oecosimu(round(funaggtab), nsimul=1000, nestedtemp, "r2dtable", parallel=4)
...
#### Mantel Tests
Tests here show the relationships between plant parts (or plant organs) and bacterial communities fungal communities across geographic distance.  The **exported table** uses Hellinger square-root trasnformed abundance data in Mantel tests with p-values and BH corrected p-values.
...
Abov.otu = subset_samples(PS_normal.hell, c(SampleType=="Air", SampleType=="Axil", SampleType=="Leaf", SampleType=="Stem", SampleType=="Petiole", SampleType=="Litter"))

Belw.out = subset_samples(PS_normal.hell, c(SampleType=="Soil", SampleType=="Root")) 
...
Abov_otu.dist = phyloseq::distance(Abov.otu, "bray")
Belw_otu.dist = phyloseq::distance(Belw.out, "bray")
...
Abov.geo = cbind(sample_data(Abov.otu)$Lon, sample_data(Abov.otu)$Lat)
Belw.geo = cbind(sample_data(Belw.out)$Lon, sample_data(Belw.out)$Lat)
...
Abov.geodist = earth.dist(Abov.geo)
Belw.geodist = earth.dist(Belw.geo)
...
Abov_bacteria_mantel = mantel(log(Abov.geodist+1),log(Abov_otu.dist), permutations= 999)
Belw_bacteria_mantel = mantel(log(Belw.geodist+1),log(Belw_otu.dist), permutations= 999)
...
fAbov.otu = subset_samples(FPS_normal.hell, c(SampleType=="Air", SampleType=="Axil", SampleType=="Leaf", SampleType=="Stem", SampleType=="Petiole", SampleType=="Litter"))

fBelw.out = subset_samples(FPS_normal.hell, c(SampleType=="Soil", SampleType=="Root"))
...
fAbov_otu.dist = phyloseq::distance(fAbov.otu, "bray")
fBelw_otu.dist = phyloseq::distance(fBelw.out, "bray")
...
fAbov.geo = cbind(sample_data(fAbov.otu)$Lon, sample_data(fAbov.otu)$Lat)
fBelw.geo = cbind(sample_data(fBelw.out)$Lon, sample_data(fBelw.out)$Lat)
...
fAbov.geodist = earth.dist(fAbov.geo)
fBelw.geodist = earth.dist(fBelw.geo)
...
fAbov_fungi_mantel = mantel(log(fAbov.geodist+1),log(fAbov_otu.dist), permutations= 999)
fBelw_fungi_mantel = mantel(log(fBelw.geodist+1),log(fBelw_otu.dist), permutations= 999)
...
mant_cor = as.data.frame(c(
  #bact
  all_bacteria_mantel$statistic, Abov_bacteria_mantel$statistic, Belw_bacteria_mantel$statistic,
  stem_bacteria_mantel$statistic,root_bacteria_mantel$statistic, 
	...
	# fungi
	fall_mantel$statistic, fAbov_fungi_mantel$statistic, fBelw_fungi_mantel$statistic,
...
p_val = c(
  #bact
  all_bacteria_mantel$signif, Abov_bacteria_mantel$signif, Belw_bacteria_mantel$signif,
  stem_bacteria_mantel$signif, root_bacteria_mantel$signif, 
  ...
  #fungi
	fall_mantel$signif, fAbov_fungi_mantel$signif, fBelw_fungi_mantel$signif,
...
results = data.frame( c( rep('Bacterial',11), rep('Fungal',11) ), 
	c( rep( c("All", "Aboveground", "Belowground", 'Stem', 'Root', 'Air', 'Leaf', 'Soil', 'Litter', 'Axil', 'Petiole'),2 ) ),
	mant_cor, p_val
	)
...
bact.cor.p = p.adjust(results[results$Type == 'Bacterial',4], method = 'BH')
bact.cor.p = data.frame(cor.p = bact.cor.p, Type = rep('Bacterial',11), 
	Part = as.character(unique(results$Part)) )

#Adjust p-values for repeated tests on the within plant part groups for fungi
fung.cor.p = p.adjust(results[results$Type == 'Fungal',4], method = 'BH')
fung.cor.p = data.frame(cor.p = fung.cor.p, Type = rep('Fungal',11), 
	Part = as.character(unique(results$Part)) )
...
results<- results[, c(1,2,4,5,3)]

#Save all result data as a csv
write.csv(results, file = 'output/Mantel_Plant_Part_Results_Hell_Bray.csv')
knitr::kable(results, digits = c(0, 0, 3, 3, 3, 3))

```

```{r, supp mantel review, include=FALSE, eval=FALSE}
## Genus level
...
PS_normal = readRDS("data/PS_normal.gz") #"PS_normal"
PS_normal<- subset_taxa(PS_normal, c(Kingdom != "unknown", Kingdom != "Eukaryota"))
PS_normal<- tax_glom(PS_normal, "Genus")
...
#Load RDS file of fungal reads
FPS_normal = readRDS("data/fungal_PS_normal.gz") #"fungal_PS_normal"
FPS_normal<- tax_glom(FPS_normal, "Genus")
...
#Save all result data as a csv
write.csv(results, file = 'output/Mantel.genus.BH.csv')
knitr::kable(results, digits = c(0, 0, 3, 3, 3, 3))
...
fun_meta_all$Ground_Type<-factor(ifelse(fun_meta_all$SampleType=="Air" | 
                                     fun_meta_all$SampleType=="Axil"| 
                                     fun_meta_all$SampleType=="Leaf" |  
                                     fun_meta_all$SampleType=="Litter" |
                                     fun_meta_all$SampleType=="Stem" | 
                                     fun_meta_all$SampleType=="Petiole", "Above.ground",
                              "Below.ground"))

fung_all = adonis(fun_ps ~ Site +Sample_Type, by = "margin", data = fun_meta_all, permutations = 10000)
fung_all

capture.output(fung_all, file="output/fung_all.permanova.csv")

fung_ab.bl = adonis(fun_ps ~ Ground_Type, by = "margin", data = fun_meta_all, permutations = 10000)
fung_all

capture.output(fung_ab.bl, file="output/fung_ab.bl.permanova.csv")
```

```{r, supp Permanova, include=FALSE, eval=FALSE}
PS_normal.genus = tax_glom(PS_normal, "Genus")

PS_normal.genus=transform_sample_counts(PS_normal.genus, function(x) x^0.5)

PS_normal.genus = subset_taxa(PS_normal.genus, c(
                          Order!="Chloroplast", 
                          Kingdom!="Archaea",
                          Family!="Mitochondria",
                          Kingdom!="Unknown",
...
#Bacterial ALL
Site <- as.factor(PS_normal.genus@sam_data$FieldSite)
Sample_Type <- PS_normal.genus@sam_data$SampleType
bact.gen_ps <- phyloseq::distance(PS_normal.genus, method="bray") #Normalized bact data - phyloseq-class
bact_meta.gen_all = as(sample_data(PS_normal.genus), "data.frame") #Extract sample data and coerce to df


bact.gen_all = adonis(bact.gen_ps ~ Site  + Sample_Type, by = "margin", 
                      data = bact_meta.gen_all, permutations = 10000)
bact.gen_all
capture.output(bact.gen_all, file="output/bact.genus.perm.csv")

######
FPS_normal.genus = tax_glom(FPS_normal, "Genus")
fun.gen_ps = transform_sample_counts(FPS_normal.genus, function(x) x^0.5)
site <- as.factor(FPS_normal.genus@sam_data$FieldSite)
SampleType <- FPS_normal.genus@sam_data$SampleType
fun.gen_ps <- phyloseq::distance(fun.gen_ps, method="bray") #Normalized bact data - phyloseq-class
fun.gen_meta_all = as(sample_data(FPS_normal.genus), "data.frame") # Extract sample data and coerce to df


fung.gen_all = adonis(fun.gen_ps ~ Site +Sample_Type, by = "margin", 
                      data = fun.gen_meta_all, permutations = 10000)
fung.gen_all
capture.output(fung.gen_all, file="output/fung.genus.perm.csv")
```
